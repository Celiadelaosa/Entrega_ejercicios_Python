...
#OBS PARTICULAR: no veo el sentido de lo que debe devolver, las palabras de la lista original que sean iguales a la
#palabra objetivo serán todas la misma palabra.Podría estar refiriéndose a que contengan una letra concreta o
#a que cuente cuántas veces aparece la palabra objetivo en mi lista. Hago ambos casos.

#CASO 1: toma una letra objetivo y devuelve las palabras que tienen esa letra
def letra_coincidente(lista, letra_objetivo):
    palabras_con_la_letra=[]     #creamos una lista vacía en la que añadiremos las palabras que contengan la letra objetivo
    for palabra in lista:
        palabra=palabra.lower()   #convertimos la palabra a minúsculas 
        if letra_objetivo in palabra:
            palabras_con_la_letra.append(palabra)
    return palabras_con_la_letra

#comprobamos que funciona
letra_coincidente(['ZAPATO', 'zarigüella', 'casa', 'comida', 'zascandil', 'rosa'], 'z')

#CASO 2: contamos cuántas veces aparece nuestra palabra en la lista
def cuenta_palabras(lista, palabra_objetivo):
    ...
    for palabra in lista:
        ...
        if palabra==palabra_objetivo:
            ...
            continue
    return 'El número de veces que aparece la palabra objetivo es {contador}' #inicialmente había usado print, pero con return podríamos reutilizar la info (el return no solo imprime como print)
...
#vamos a pedir que se genere un mensaje si las listas no tienen la misma longitud para evitar confusiones
lista_1=[0,1,2,4,5,6,14] #proporcionamos dos listas 
lista_2=[1,2,3,7,8,8,9]
resultado=list(map(resta,lista_1, lista_2)) #definimos el resultado fuera del bucle
...
if len(lista_1)!=len(lista_2):              #si las listas no tienen la misma longitud generamos un mensaje
    ...
    print(resultado)                   #uso print para que me 'printee' la lista
...
#inicialmente había escrito este código, pero lanzaba un mensaje, no hacía exactamente lo que me pide el enunciado, que es lanzar una excepción
def calculo_promedio(lista_numeros):
    if len(lista_numeros)==0:
        print('La lista está vacía')    #si la lista está vacía lanza un mensaje
    else:
        print(sum(lista_numeros)/len(lista_numeros))  #si no,calcula el promedio de los elementos de la lista
...
def paso_a_tupla(caracter):                   #defino una función que,al recibir una letra, devuelve una tupla con la letra en mayúscula y minúscula
    return(caracter, caracter.upper())
...
def filtrado_inicial(palabra, inicial='p'):  #esta función devuelve true si la palabra empiezan por p (podremos cambiar esta inicial según la que nos interese mirar)
    palabra_minuscula=list(palabra.lower()) #paso la palabra a minúsculas y uso list para pasar de formato str a formato lista
    return(palabra_minuscula[0]==inicial)  #ahora puedo usar índices y para tomar la inicial de la palabra uso el índice 0.Me devuelve true si la inicial coincide 
...
#creamos ahora una lista de duplas que contengan el valor del estudiante y su nota para cada uno de los diccionarios
lista_de_duplas=[]
for diccionario in lista_diccionarios:
    lista_de_duplas.append((tuple((diccionario.get("nombre"), diccionario.get("calificación")))))

#en filter tomamos como función una tome tuplas y devuelva true si el segundo elemento de la tupla (índice 1) es >=90
print(list(filter(lambda tupla: tupla[1]>=90, lista_de_duplas)))  #el output es una lista con las tuplas (alumno, calificación) donde la calificación sea >=90
...
def cuenta_caracteres_2(texto):
    return(len(texto.split()))                         #con split tenemos una lista con las palabras de nuestra cadena de texto

#si queremos simplificar la función
cuenta_caracteres_simple=lambda texto:len(texto.split())
print(cuenta_caracteres_simple('que buen dia hace hoy en Madrid'))
...
lista=[2,3,4,5,6,7,8,89,3,2,4]     #proporcionamos una lista
...
for elemento in lista:             #recorro mi lista dada
    if elemento not in lista_2:    #si el elemento de la lista no esá en la lista (lo que ocurre si todavía no lo he añadido)
        ...
        print(elemento)            #devuelvo ese elemento
        break                      #y rompo el bucle (salgo de él). Esto se lo pido porque solo quiero el primer elemento duplicado

if len(lista)==len(lista_2):       #y ahora, si he añadido todos los elementos de la lista a la lista_2 (con lo que tienen la misma longitud) significa que ninguno estaba repetido
    print('No había elementos repetidos en la lista proporcionada')
...
def encuentra_nombre():
    lista_nombres=input('Dame una lista de nombres escritos en minúsculas')
    nombre=input('¿Qué nombre quieres que encuentre?')
    if nombre in lista_nombres:
        print(f'El nombre {nombre.capitalize()} está en la lista')
    else:
        print(f'El nombre {nombre.capitalize()} NO está en la lista')
...
#no trabaja aquí.
def encuentra_empleado():
    nombre=input('Dame un nombre completo')
    cargo=input('Dime su cargo en la empresa')#añado esta pregunta al usuario para poder obtener el puesto del trabajador
    lista_empleados=input('Dame una lista de empleados')
    if nombre in lista_empleados:
        print(f'El puesto de {nombre.capitalize()} es {cargo}.')
    else:
        print(f'La persona {nombre.capitalize()} no trabaja aquí.')
...
UsuarioBanco.agregar_dinero(Bob, 20)
UsuarioBanco.transferir_dinero(Alicia, Bob, 80)
UsuarioBanco.retirar_dinero(Alicia, 50)
...
def reemplazar_palabra(texto,palabra_a_reemplazar, palabra_nueva):
    if palabra_a_reemplazar in texto:
        ...
        print(f'La palabra {palabra_a_reemplazar} no está en el texto.')
    return nuevo_texto
...
if 0<=nota_numerica<=69:
    print ('Insuficiente')
if 70<=nota_numerica<=79:
    print ('Bien')
if 80<=nota_numerica<=89:
    print ('Muy bien')
if 90<=nota_numerica<=100:
    print ('Excelente')
...
def area_figura(figura, datos):            #datos es la tupla
    if figura=='triángulo':
        ...
        radio=datos[0]
        print(f'El área del círculo es {math.pi*(radio**2)}.')   #en el caso del cículo solo hace falta un argumento
    else:
